"""Ranking & Final Production Model Selection — Phase 7B end to end.

Sequences the stages this phase owns:

    Ranking
        -> Dynamic Drift Selection -> Threshold Validation
        -> Drift Validation -> Winner Selection

Model Ranking's SHAP/importance input is generated by a preceding stage
(`forecast_engine.s07_explainability`, Section 6.10) and handed in here as a
finished `ExplainabilityReport` — this class does not generate it.

`ModelRanker` and `ProductionModelSelector` each already isolate failures at
their own grain (per group); this class only sequences the two so
`run_pipeline.py` has one call to make for the whole phase.
"""
...
class ProductionSelectionPipeline:
    ...
    def run(
        ...
        """Rank the surviving candidates, then select a winner per group.

        Ranking and selection are one stage because selection consumes the
        ranked order directly: it walks candidates best-first until one passes
        drift validation. Splitting them would mean persisting an intermediate
        ranking no other stage reads.
        """
